...
|     a while-loop (see the chapter "Loops"). At each step compare
|     the old sum with the new sum. If the difference between
|     the two sums Math.Abs(current_sum – old_sum) is less than
|     the required precision (0.001), the calculation should finish
|     because the difference is constantly decreasing and the precision
|     is constantly increasing at each step of the loop.
