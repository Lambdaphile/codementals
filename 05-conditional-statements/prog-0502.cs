...
|     if the negative multiples are even number.
|     If the count of the negative numbers is odd, the product
|     is negative. If at least one of the numbers is zero,
|     the product is also zero.
|
|     Use a counter negativeNumbersCount to keep the number of
|     negative numbers. Check each number whether it is negative
|     and change the counter accordingly. If some of the numbers
|     is 0, print "0" as result (the zero has no sign).
|     Otherwise print "+" or "-" depending on the condition
