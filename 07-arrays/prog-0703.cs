...
|
|     In lexicographic order the elements are compared one by
|     one starting from the very left. If the elements are not
|     the same, the array, whose element is smaller (comes
|     earlier in the alphabet), comes first. If the elements are
|     equal, the next character is compared. If the end of one of
|     the arrays is reached, without  finding different elements,
|     the shorter array is the smaller (comes earlier
|     lexicographically).
